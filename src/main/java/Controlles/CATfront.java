...
public class CATfront implements Initializable {
    ...
    private void navigateToActivitesForCategorie(Categories cat) {
        ...
        navigateTo("/activitesfront.fxml", "TravelMate - Activités : " + cat.getNom(),
                btnVersActivites.getScene().getWindow());
    }

    @FXML
    private void handleVersActivites() {
        CategorieContext.categorieFiltre = null;
        navigateTo("/activitesfront.fxml", "TravelMate - Activités",
                btnVersActivites.getScene().getWindow());
    }

    @FXML
    private void handleToggleOffice() {
        navigateTo("/categoriesback.fxml", "TravelMate - Back Office Catégories",
                btnToggleOffice.getScene().getWindow());
    }

    private void navigateTo(String fxml, String title, javafx.stage.Window window) {
        try {
