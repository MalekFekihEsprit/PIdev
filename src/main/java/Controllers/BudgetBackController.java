...
public class BudgetBackController implements Initializable {
    ...
    @FXML private HBox btnParametres;
    ...
    private void updateAnalyseFinanciere() {
        ...
        if (voyageId > 0 && voyagesMap != null && voyagesMap.containsKey(voyageId)) {
            ...
            // VoyageInfo expose getDateDebut() / getDateFin() retournant java.util.Date ou LocalDate
            // Adapte le cast selon ta classe VoyageInfo réelle :
            try {
        ...
        if (dureeJours == 1 && !toutesDepenses.isEmpty()) {
            java.sql.Date dMin = toutesDepenses.stream()
                    .map(Depense::getDateCreation).min(Comparator.naturalOrder()).orElse(null);
            java.sql.Date dMax = toutesDepenses.stream()
    ...
    private void setupNavigation() {
        navigateOnClick(btnDestinations, "Destinations");
        navigateOnClick(btnItineraires,  "Itinéraires");
        navigateOnClick(btnActivites,    "Activités");
        navigateOnClick(btnHebergements, "Hébergements");
        navigateOnClick(btnBudgets,      "Budgets");
        navigateOnClick(btnParametres,   "Paramètres");
        navigateOnClick(btnHome,         "Accueil");
    }
    private void navigateOnClick(HBox btn, String page) {
        if (btn != null) btn.setOnMouseClicked(e -> System.out.println("Navigation : " + page));
    }
