    ...
    private void setupNavigationButtons() {
        ...
        if (btnHome != null) {
            ...
            btnHome.setOnMouseExited(event -> {
                btnHome.setStyle("-fx-background-color: #1e2749; -fx-background-radius: 12; -fx-min-width: 40; -fx-min-height: 40; -fx-cursor: hand;");
                btnHome.lookupAll(".label").forEach(label -> {
                    if (label instanceof Label) {
                        ((Label) label).setStyle("-fx-font-size: 18; -fx-text-fill: #94a3b8;");
                    }
                });
            });
        }
    ...
    private void handleBulkDelete() {
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            ...
            if (failCount == 0) {
                ...
                for (String name : successfulNames) {
                    successList.append("✓ ").append(name).append("\n");
                }
                showAlert(Alert.AlertType.INFORMATION, "Succès",
                ...
                for (String name : failedNames) {
                    failList.append("✗ ").append(name).append("\n");
                }
