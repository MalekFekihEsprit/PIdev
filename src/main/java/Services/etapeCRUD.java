...
public class etapeCRUD implements interfaceCRUDItine<etape> {
    ...
    public void ajouter(etape etape) throws SQLException {
        // AJOUTER numero_jour dans la requête
        ...
        pst.setInt(5, etape.getNumeroJour()); // AJOUT : insérer le numéro du jour
        pst.executeUpdate();
    ...
    public void modifier(etape etape) throws SQLException {
        // AJOUTER numero_jour dans la requête de mise à jour
        ...
        pst.setInt(4, etape.getNumeroJour()); // AJOUT : mettre à jour le numéro du jour
    ...
    public List<etape> getEtapesByItineraire(int id_itineraire) throws SQLException {
        ...
        while (rs.next()) {
            etapes.add(extractEtapeFromResultSet(rs));
        }
        return etapes;
    }
    ...
    private etape extractEtapeFromResultSet(ResultSet rs) throws SQLException {
        ...
        // Lire numero_jour
        try {
            e.setNumeroJour(rs.getInt("numero_jour"));
        } catch (SQLException ex) {
            e.setNumeroJour(1); // Valeur par défaut si la colonne n'existe pas
        }
        ...
        // CORRECTION: Lire comme String puis convertir en Integer
    ...
    public List<ActiviteItem> getActivitesByVoyage(int idVoyage) throws SQLException {
        ...
        while (rs.next()) {
            // Récupérer niveau_difficulte comme String, pas comme int
            String niveauDifficulte = rs.getString("niveaudifficulte");

            // Convertir le niveau en entier pour votre constructeur
            int niveauInt = 0;
            ...
            activites.add(new ActiviteItem(
                    ...
                    rs.getFloat("duree"),
                    rs.getFloat("budget"),
                    niveauInt  // Utiliser l'entier converti
            ));
        }
